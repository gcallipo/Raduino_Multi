...
#define MULTI_BAND
  ...
  #define BAND_40   (7100000UL) 
  #define BAND_20  (14200000UL) 
  #define BAND_15  (21200000UL) 
  #define BAND_10  (28400000UL) 

  long bandVFOA = BAND_40;
  long bandVFOB = BAND_20;
...
*/
#ifdef MULTI_BAND
    /* Increase buffer seats to allow display high frequency */
	char c[18], b[11], printBuff1[18], printBuff2[18];
#else
...
#ifdef MULTI_BAND
    /* Wide LO and HI frequencies bound */
	#define LOWEST_FREQ  (1800000UL) // absolute minimum frequency (Hz)
	#define HIGHEST_FREQ (30000000UL) //  absolute maximum frequency (Hz)
#else
...
#ifdef MULTI_BAND
/* Define simple band change routine */

void bandChange(){
  long oldBand,newBand;
  byte newMode;
  
  // which VFO (false=A or true=B) is active
  if (!vfoActive){
    oldBand = bandVFOA;
  }else{
    oldBand = bandVFOB;
...
/**
   ...
   2 short presses: toggle RIT on/off  (BAND CHANGE) if multiband
...
void checkButton() {
  ...
  if (digitalRead(FBUTTON)) {
    ...
    if (!digitalRead(FBUTTON)) {
      ...
      #ifdef MULTI_BAND
        /* no rit in multiband operation  ... we need to check */
      #else
  ...
  switch (action) {
    ...
    #ifdef MULTI_BAND
       /* in Multiband operatin 2 click for band change */
      bandChange();
    #else
      toggleRIT();
    #endif  
...
#ifdef MULTI_BAND
/* Avoid reset VFO for now ... we need to check it in multiband */
#else
...
#ifdef MULTI_BAND  
  EEPROM.put(20, 14125000UL); // VFO B frequency (7125 kHz)
  EEPROM.put(24, USB); // mode VFO A (LSB)
  EEPROM.put(25, LSB); // mode VFO B (USB)
#else
  EEPROM.put(20, 7125000UL); // VFO B frequency (7125 kHz)
  EEPROM.put(24, LSB); // mode VFO A (LSB)
  EEPROM.put(25, LSB); // mode VFO B (LSB)
#endif  
...
#ifdef MULTI_BAND  
/* Disable autosave if multiband ... we need to check */
#else
...
void setup() {
  ...
  #ifdef MULTI_BAND
  /* over write the values on startup and do not read from eeprom. */
     vfoA= 7125000UL;  
     vfoB=14200000UL;
     mode_A=LSB;
     mode_B=USB;
